...
## JavaFX 三路搜索融合（2026-09-25）

- 恢复 SQL `LIKE '%query%'`（保留 `%` / `_` 语义），直接搜索已同步的本地笔记；过滤空正文和解密失败。SQL / keyword / semantic 按固定 `3 / 2 / 1` 权重做 RRF（k=60），没有 SQL 置顶或来源分组；同分按时间、ID 倒序，融合去重后最多 100 条。
- SQL 命中不显示 tag，包括同时命中 KS / SS 的笔记；其余保留 KS / SS。后台 hydration 保留融合顺序，替换此前全局时间倒序；关闭语义时保留 SQL + keyword。
- 每路候选上限仍为 100。SQL wildcard 需要扫描正文，沿用后台 Task 执行；本次不改变数据库 schema 或 Lucene 格式，无需重建索引。
- 验证：`mvn -q -Dkeenotes.search.fx.tests=true test`，33 项通过（含 8 项 JavaFX 回归）；容量与侧栏 opt-in 用例未启用。修正旧 FX 用例共用查询词造成的样本干扰，使用独立查询词。
