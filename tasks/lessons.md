...
**规则**:
1. 新增列表类 UI 时，必须先检查现有的列表组件（Desktop: `NoteCardView`/`NotesDisplayPanel`，iOS: `NoteRow`，Android: `item_note.xml`/`NotesAdapter`），优先复用
...
4. 如果需要数据类型转换来复用组件（如 `PendingNote → Note`），添加 `toXxx()` 转换方法，不要为了省事自己写一套新 UI
