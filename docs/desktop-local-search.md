...
- 只有原来的一个搜索入口，没有关键词/混合模式选择。关键词查询统一改用 Lucene + Java jieba（索引 INDEX、查询 SEARCH），删除原 SQL LIKE 搜索。
...
- 没配置或未启用时只查询关键词，不调用模型。启用后，先显示关键词结果，再补充 hybrid 结果；服务失败或超时保留关键词结果。连接测试只发送固定测试文本。
...
- 初次使用需要点击全量按钮，历史笔记才会全部覆盖。新增同步笔记持续进入后台增量索引；全量尚未完成时，搜索显示覆盖范围提示。

## 模块与落点

| 模块 | 职责 |
| --- | --- |
| `LocalCacheService` / `SearchStore` | 在远程同步写入同一 SQLite 事务内记录索引任务；保存重建快照、配置代次及失败状态 |
...
索引目录使用**实际缓存数据库**的父目录：`search/<epoch>/{keyword,vector}/<profile>/`。正常默认路径为 `~/.keenotes/search/`。数据库仍保存笔记；Lucene 只存搜索所需字段。召回保留相关性排名和最多 100 条的候选范围；按 note ID 回查 SQLite 后，桌面结果列表按 `created_at` 倒序展示，同时间按 ID 倒序，缺失时间放最后。关键词预览和混合搜索最终结果使用相同的展示排序。
...
8. **查询一致性**：每一路在合并 Base/Delta 的同一 Reader 视图中排名，按 note ID 和输入版本遮蔽旧副本；全量发布时对照当前源记录剔除早于快照的旧 Delta，保留构建期间的新 Delta。两路再按 note ID 做等权 RRF（`k=60`），返回最多 100 条；Base/Delta 不作为四路信号融合。
...
自动化用例覆盖：中文长词和英文大小写、无 provider 查询、提交后增量可见、低 ID 补入、解密恢复、全量固定范围、取消/重启恢复、向量复用、HTTP 取消及响应 index 校验、失败重试、模型切换覆盖、旧 profile 停用、旧账号文件清理、RRF 去重和凭据恢复。
